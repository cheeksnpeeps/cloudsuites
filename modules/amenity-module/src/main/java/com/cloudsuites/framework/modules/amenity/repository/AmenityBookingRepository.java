...
public interface AmenityBookingRepository extends JpaRepository<AmenityBooking, String> {
    ...
    @Transactional
    @Query("SELECT b FROM AmenityBooking b WHERE b.userId = :userId AND (:type IS NULL OR b.amenity.type = :type) AND (:startDate IS NULL OR b.startTime >= :startDate) AND (:endDate IS NULL OR b.endTime <= :endDate)")
