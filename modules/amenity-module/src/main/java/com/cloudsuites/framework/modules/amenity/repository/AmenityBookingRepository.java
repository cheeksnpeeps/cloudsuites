...
public interface AmenityBookingRepository extends JpaRepository<AmenityBooking, String> {
    ...
    @Transactional
    @Query("SELECT b FROM AmenityBooking b WHERE (:userId IS NULL OR b.userId = :userId) " +
            "AND (:amenityId IS NULL OR b.amenity.amenityId = :amenityId) " +
            "AND (:type IS NULL OR b.amenity.type = :type) " +
    ...
    @Query("SELECT COUNT(b) FROM AmenityBooking b WHERE b.amenity.amenityId = :amenityId AND b.startTime < :endTime AND b.endTime > :startTime")
